...
const passwords = lines.map(x => {
    ...
    return {req_let: req_let, min: min, max: max, password: divide[2], isvalid: undefined};
});
...
passwords.forEach((element,index) => {
    ...
    const nOfreq_let = split.filter(x => {
        return x === element.req_let;
    }).length;

    if(element.min <= nOfreq_let && nOfreq_let <= element.max){
