const input_data = `3-11 z: zzzzzdzzzzlzz
3-7 x: xjxbgpxxgtx
3-4 v: vvmv
3-5 t: tgkfq
...
const passwords = lines.map(x => {
    const divide = x.split(" ");

    const req_let = divide[1].slice(0,1);

    const minMax = divide[0].split("-")

    const min = parseInt(minMax[0])
    const max = parseInt(minMax[1]);


    return {req_let: req_let, min: min-1, max: max-1, password: divide[2], isvalid: undefined};
});

passwords.forEach((element,index) => {

    const split = element.password.split("");

    if(((split[element.min] === element.req_let) || (split[element.max] === element.req_let)) && (!((split[element.min] === element.req_let) && (split[element.max] === element.req_let)))){
        passwords[index].isvalid = true;
    } else {
        passwords[index].isvalid = false;
    }
