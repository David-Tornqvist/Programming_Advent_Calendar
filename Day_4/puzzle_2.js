const raw_data = `eyr:2028 iyr:2016 byr:1995 ecl:oth
...
const split = raw_data.split("\n");
let indexOfNewPassport = [];

for (let i = 0; i < raw_data.length; i++) {
    if(split[i] === ""){indexOfNewPassport.push(i)};
}
...
let concatedPasswords = passwords.map(x => {
    return x.join(" ");
});



let categorizedpaswords = concatedPasswords.map(x => {
    return x.split(" ");
})


let forPasswrd = categorizedpaswords.map(x => {
    let passwrd = { byr: undefined,
                    iyr: undefined,
                    eyr: undefined,
                    hgt: undefined,
                    hcl: undefined,
    ...
    x.forEach(x => {
        ...
        if(value[0] === "byr"){passwrd.byr = value[1]};
        if(value[0] === "iyr"){passwrd.iyr = value[1]};
        if(value[0] === "eyr"){passwrd.eyr = value[1]};
        if(value[0] === "hgt"){passwrd.hgt = value[1]};
        if(value[0] === "hcl"){passwrd.hcl = value[1]};
        if(value[0] === "ecl"){passwrd.ecl = value[1]};
        if(value[0] === "pid"){passwrd.pid = value[1]};
    });
...
const validPasswrds = forPasswrd.filter(x => {
    return (x.byr && x.iyr && x.eyr && x.hgt && x.hcl && x.ecl && x.pid)
});

const correctPasswrds = validPasswrds.filter(x => {
    const byr = (1920 <= x.byr) && (x.byr <= 2002);
    const iyr = (2010 <= x.iyr) && (x.iyr <= 2020);
    const eyr = (2020 <= x.eyr) && (x.eyr <= 2030);


    let hgt;
    const height = x.hgt.split("");
    if(height.includes("c")){hgt = (150 <= parseInt(x.hgt)) && (parseInt(x.hgt) <= 193)};
    if(height.includes("i")){hgt = (56 <= parseInt(x.hgt)) && (parseInt(x.hgt) <= 76)};


    let hairColor = x.hcl.split("");
    const istag = (hairColor[0] === "#");
    const corlength = (hairColor.length === 7);

    hairColor[0] = "";
    isNumLet = hairColor.join("").match("^[A-Fa-f0-9]+$");

    hcl = istag && corlength && isNumLet;

    ecl = (x.ecl === "amb") || (x.ecl === "blu") || (x.ecl === "brn") || (x.ecl === "gry") || (x.ecl === "grn") || (x.ecl === "hzl") || (x.ecl === "oth");

    pid = (x.pid.split("").length === 9) && x.pid.match("^[0-9]+$"); 

    return byr && iyr && eyr && hgt && hcl && ecl && pid; 
    });

console.log(validPasswrds);
console.log(validPasswrds.length);
console.log(correctPasswrds);
console.log(correctPasswrds.length);
