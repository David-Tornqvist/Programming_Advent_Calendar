...
const AnsweredYes = answers.map(x => {
    let yeses = [];

    x.forEach(y => {
        const letters = y.split("");
            letters.forEach((z) => {
                if(!yeses.includes(z)){yeses.push(z)};
            });
    });

    return {AnsweredYes: yeses.sort(),nOfYes: yeses.length}
});

console.log(AnsweredYes);

let i = 0;
const sumOfYes = AnsweredYes.forEach(x => {
    i += x.nOfYes;
});
console.log(i);
